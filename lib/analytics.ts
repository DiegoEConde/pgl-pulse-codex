...
export function filterAnalytics(rows: AnalyticsRow[], config: ReportConfig) { return rows.filter(row => config.state === "TODOS" || row.state === config.state); }
export function metricValue(row: AnalyticsRow, metric: Metric) {
  ...
  return row.state === "REPARTO" || row.state === "ENTREGADA" ? row.sale - row.cost - row.commission : 0;
}
export function aggregate(rows: AnalyticsRow[], config: ReportConfig): ChartDatum[] {
  ...
  return [...grouped].map(([label,value])=>({label,value})).sort((a,b)=>config.dimension==="month"?a.label.localeCompare(b.label):b.value-a.value);
}
export function formatMetric(value:number, metric:Metric) {
  return metric === "units" ? String(value) : new Intl.NumberFormat("es-AR",{style:"currency",currency:"USD",minimumFractionDigits:2,maximumFractionDigits:2}).format(value);
}
