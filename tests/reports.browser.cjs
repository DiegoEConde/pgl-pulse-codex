...
(async()=>{
 ...
 try{
  ...
  await pdf(page.getByRole("button",{name:"Exportar a PDF",exact:true}),"tests/reports-current.pdf");
  ...
  await pdf(result.getByRole("button",{name:"Exportar a PDF",exact:true}),"tests/reports-custom.pdf");
  ...
  await page.evaluate(()=>window.scrollTo(0,0));await page.screenshot({path:"tests/reports-mobile.png",fullPage:true});await page.setViewportSize({width:1440,height:900});await page.screenshot({path:"tests/reports-desktop.png",fullPage:true});
