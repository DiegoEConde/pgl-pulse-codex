...
export default function AnalyticsChart({data,type,metric}:{data:ChartDatum[];type:ChartType;metric:Metric}){
  ...
  if(type==="bar")return <div className={`${styles.chart} ${styles.bars}`}>{data.slice(0,8).map((item)=><div className={styles.bar} key={item.label}><span>{item.label}</span><div className={styles.track}><div className={styles.fill} style={{width:`${Math.abs(item.value)/max*100}%`}}/></div><b className={styles.value}>{formatMetric(item.value,metric)}</b></div>)}</div>;
  if(type==="line"){
    const ordered=data.slice(-8);const points=ordered.map((item,index)=>`${20+index*(560/Math.max(ordered.length-1,1))},${110-item.value/max*80}`).join(" ");
  ...
  if(data.some(item=>item.value<0)||data.every(item=>item.value===0)) return <div className={styles.empty}>La dona necesita valores positivos. Usá barras o línea para este informe.</div>;
