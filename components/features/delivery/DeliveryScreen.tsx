...
export default function DeliveryScreen() {
  ...
  const delivered = sales.filter(sale => sale.status === "ENTREGADA").sort((a,b) => (b.deliveredAt ?? "").localeCompare(a.deliveredAt ?? ""));
  ...
  return <div className={`view ${layout.page}`}>
    ...
    <WorkspaceTabs labels={["Pendientes", "Cierre del día", "Entregadas"]}>    <section className="panel"><header className="panel-head"><h2>Entregas pendientes</h2><span className="badge blue">{filtered.length} unidades</span></header><div className="table-wrap">{filtered.length ? <PagedTable><thead><tr><th>Unidad</th><th>Producto</th><th>Cliente</th><th>Dirección</th><th>Teléfono</th><th>Pago</th><th>Acciones</th></tr></thead><tbody>{filtered.map(sale => {
      const client = clients.get(sale.clientId);
    ...
    })}</tbody></PagedTable> : <div className={styles.empty}>No hay entregas pendientes que coincidan.</div>}</div></section>
