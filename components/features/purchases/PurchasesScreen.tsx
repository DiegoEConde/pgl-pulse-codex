...
import { purchaseOptions, initialPurchaseOptions } from "@/lib/purchase-options";
...
export default function PurchasesScreen() {
  ...
  const options = raw.purchaseOptions ?? initialPurchaseOptions;
  const fieldsFor = (key: number) => purchaseOptions(options, raw.products.find(product => product.id === selectedProducts[key]));
  ...
  function create(event: FormEvent<HTMLFormElement>) {
    ...
    if (!raw.purchaseOptions) { setError("Las opciones de compra todavía no están disponibles para guardar."); return; }
  ...
  return <div className={`view ${layout.page}`}>
    <PageHeader title="Compras" action={<button className={`primary-btn ${layout.headAction}`} disabled={!raw.products.length || !raw.suppliers.length} onClick={() => { requestId.current = crypto.randomUUID(); setLineKeys([++lineCounter.current]); setSelectedProducts({}); setCreating(true); setNotice(""); setDirty(false); setError(""); }}><Plus size={16} /> Nueva compra</button>} />
    ...
    {creating && <div className={styles.modalOverlay}><section className={styles.modal} role="dialog" aria-modal="true" aria-label="Nueva compra"><header className={styles.modalHeader}><h2>Nueva compra</h2><button className={styles.close} aria-label="Cerrar compra" disabled={busy} onClick={close}><X size={20} /></button></header>
      ...
      {lineKeys.map((key,index) => <div className={styles.receiptLine} key={key}><h3>Producto {index + 1}</h3><div className={styles.formGrid}>
        <div className={styles.field}><label htmlFor={"product-" + key}>Producto</label><select id={"product-" + key} name={"product-" + key} required defaultValue="" onChange={event => setSelectedProducts(current => ({ ...current, [key]: Number(event.target.value) }))}><option value="" disabled>Seleccionar producto</option>{raw.products.map(item => <option key={item.id} value={item.id}>{item.marca} · {item.nombre}</option>)}</select></div>
        {fieldsFor(key).map(field => <div className={styles.field} key={field.clave + "-" + (selectedProducts[key] ?? "")}><label htmlFor={(field.clave === "color" ? "color-" : "option-" + field.clave + "-") + key}>{field.etiqueta}</label><select id={(field.clave === "color" ? "color-" : "option-" + field.clave + "-") + key} name={(field.clave === "color" ? "color-" : "option-" + field.clave + "-") + key} required disabled={!selectedProducts[key] || !field.valores.length} defaultValue=""><option value="" disabled>{field.valores.length ? "Seleccionar" : "Opciones pendientes de cargar"}</option>{field.valores.map(value => <option key={value} value={value}>{value}</option>)}</select></div>)}
        {selectedProducts[key] && fieldsFor(key).every(field => field.clave === "color") && <p className={styles.formNote}>Las variantes de esta categoría se cargarán más adelante.</p>}
      ...
      {!raw.purchaseOptions && <p role="status">Las opciones de compra todavía no están disponibles para guardar.</p>}
      <p className={styles.formNote}>La compra quedará pendiente en Repartos. Podés continuar cargando otros pedidos.</p>{error && <p className="operation-error" role="alert">{error}</p>}
      </div><footer className={styles.modalActions}><button type="button" className={styles.cancel} onClick={close}>Cancelar</button><button className="primary-btn" type="submit" disabled={!raw.purchaseOptions}>{busy ? "Guardando…" : "Cargar en Reparto"}</button></footer></fieldset></form>
    </section></div>}
  </div>;
}
